...
basedf <- read_parquet(paste0(TCDIR, "base2021.parquet"),
  ...
  mutate(agibase=c00100, 
         taxstatus=ifelse(iitax > 0, "taxable", "nontaxable"),
         numret=1, type="base")
...
usdata <- data3 %>%
  ...
  select(taxstatus, stubnum, stub, measure, base, reform, change, pchange) %>%
  pivot_longer(-c(taxstatus, stubnum, stub, measure)) %>%
  unite("name", c(measure, name)) %>%
  pivot_wider() %>%
  mutate(junk="") %>%
  arrange(stubnum) %>%
  select(taxstatus, stub, starts_with("wtdsum"), junk, starts_with("avg"))
...
tabdata <- usdata %>%
  filter(taxstatus=="allfilers") %>%
  select(stub, starts_with("wtdsum"), -wtdsum_pchange) %>%
...
tabdata <- usdata %>%
  filter(taxstatus=="taxable")  %>%
  select(stub, starts_with("avgval"))
...
mapdata <- data3 %>%
  filter(stabbr %in% state.abb, stubnum==0, variable=="iitax", measure=="avgval", taxstatus=="taxable") %>%
...
```{r include=TRUE, fig.width=8, fig.height=6.5}
df <- data3 %>%
  filter(!stabbr %in% c("US", "other"), stubnum==0, measure=="avgval", variable=="iitax", taxstatus=="taxable") %>%
  select(stabbr, change) %>%
  mutate(change=-change) %>%
  ungroup
mdn <- median(df$change, na.rm=TRUE)
...
p <- df %>% 
  ...
  # labs(caption="\nGrey vertical line marks median value") +
...
df <- data3 %>%
  filter(!stabbr %in% c("US", "other"), stubnum==0, measure=="avgval", variable=="iitax", taxstatus=="taxable") %>%
  select(stabbr, change) %>%
  add_row(stabbr="-- Median state --", change=NA_real_) %>%
  ungroup %>%
  mutate(change=ifelse(stabbr=="-- Median state --", median(change[stabbr != "-- Median state --"], na.remove=TRUE), change)) %>%
  mutate(change=-change)
...
statetab <- function(stabbr){
  tabdata <- data3 %>%
    filter(stabbr==!!stabbr, measure=="avgval", variable=="iitax", taxstatus=="taxable") %>%
    arrange(stubnum) %>%

...
st_tab <- function(st, var, data=data1){
  df <- data %>%
    filter(stabbr==st, variable==var)
  df %>%
    select(type, stubnum, stub, avgval) %>%
    pivot_wider(names_from=type, values_from=avgval) %>%
    mutate(change=reform - base,
           pchange=ifelse(base > 0, change / base, NA_real_))
}
...
data3
df <- data3 %>%
  filter(stabbr=="US", variable=="iitax", wtype=="REWT2021", measure=="wtdsum") %>%
  select(stub, base, reform, change, pchange)
vartab(df, title="abvc", dollar_digits=1)
...
## Map of average change in iitax
...
p <- ggplot(data = mdata, aes(x = x, y = y, group = group)) +
  ...
  theme(legend.position = "right") +
  theme(legend.title = element_blank()) + 
  ggtitle("Repeal cap on SALT deduction",
          subtitle="Change in average federal income tax per return")
...
p <- df %>% 
  ...
  theme_minimal() +
  theme(axis.text.y = element_text(hjust = 0)) +
  ggtitle("Impact of repealing the SALT cap, average reduction per return (denominator=all returns)",
          subtitle="Compared to current 2021 law")
p
...
# Appendix: State-specific tables

...
vars <- c("RECID", "c00100", "e00200",  "standard", "c04470", "c18300",  "iitax", "combined")

basedf <- read_parquet(paste0(TCDIR, "base2021.parquet"),
...
reformdf <- read_parquet(paste0(TCDIR, "reform2021.parquet"),
                       col_select = all_of(vars)) %>%
...
wsums <- wstack %>%
  ...
  select(-c(RECID, filer2017, numret, 
            c00100, e00200, c18300, c04470, standard, combined, iitax, agibase)) %>%
...
stiitax <- data3 %>%
  filter(measure %in% c("wtdsum", "avgval"), variable=="iitax") %>%
  select(stabbr, taxstatus, stubnum, stub, measure, base, reform, change, pchange) %>%
  pivot_longer(-c(stabbr, taxstatus, stubnum, stub, measure)) %>%
  unite("name", c(measure, name)) %>%
  pivot_wider() %>%
  mutate(junk="") %>%
  arrange(stubnum) %>%
  select(stabbr, taxstatus, stubnum, stub, starts_with("wtdsum"), junk, starts_with("avg"))
...
tab <- tabdata %>%
  ...
  gt()  %>%  
  tab_header(
    title = html("SALT-cap repeal<br>Billions of dollars"),
    subtitle = "Compared to current 2021 law"
  ) %>%
  cols_label(
      stub = "",
      wtdsum_base = "2021 current law",
      wtdsum_reform = "SALT-cap repeal",
      wtdsum_change = html("Repeal minus<br>current law"),
...
# configure save settings for wide table
gtsave(tab, "USImpact_billions_table.png", path = here::here("results"), zoom=1.5, vwidth=1500)  # zoom 2 default
write_csv(tabdata, here::here("results", "USImpact_billions_table_data.csv"))
...
tab <- tabdata %>%
  ...
  gt()  %>%  
  tab_header(
    title = html("SALT-cap repeal for the United States<br>Average impact per taxpayer"),
    subtitle = "Compared to current 2021 law"
  ) %>%
  cols_label(
      stub = "",
...
# configure save settings for wide table
gtsave(tab, "USImpact_averages_table.png", path = here::here("results"), zoom=1.5, vwidth=1500)  # zoom 2 default
write_csv(tabdata, here::here("results", "USImpact_averages_table_data.csv"))
...
write_csv(tabdata, here::here("results", "states_ranked_table_data.csv"))
...
tabdata <- df1 %>%
  select(stabbr, totchange=change) %>%
  ...
  select(stname, stname, totchange, avgchange, avgpchange)


tab <- tabdata %>%
  gt()  %>%  
  tab_header(
    title = "Impact of SALT-cap repeal for the United States and for individual states",
    subtitle = "Compared to current 2021 law"
  ) %>%
  cols_label(
    stname="",
    totchange = html("Change in $ billions: <br>Repeal minus<br>current law"),
...
# configure save settings for wide table
gtsave(tab, "states_detailed_table.png", path = here::here("results"), zoom=2, vwidth=1500)  # zoom 2 default
write_csv(tabdata, here::here("results", "states_detailed_table_data.csv"))
...
write_csv(mapdata, here::here("results", "avg_iitax_change_map_data.csv"))
...
p <- ggplot(data = mdata, aes(x = x, y = y, group = group)) +
  ...
  ggtitle("Impact of repealing the SALT cap, change in federal income tax, $ millions",
          subtitle="Compared to current 2021 law")
p
ggsave(plot=p, filename=here::here("results", "total_iitax_change_map.png"), width=8, height=6.5, units="in")
write_csv(mapdata, here::here("results", "total_iitax_change_map_data.csv"))
...
write_csv(mapdata, here::here("results", "iitax_change_hbars_data.csv"))

```


```{r include=TRUE, fig.width=8, fig.height=6.5}
# barchart with text lables
df <- data3 %>%
  filter(!stabbr %in% c("US", "other"), stubnum==0, measure=="wtdsum", variable=="iitax", taxstatus=="allfilers") %>%
  select(stabbr, change) %>%
  mutate(change=-change / 1e9,
...
p <- df %>% 
  ...
  annotate(geom="text", x=mdn + .1, y=5, label="median", hjust=0) 
p
ggsave(plot=p, filename=here::here("results", "iitax_change_hbars_numbers.png"), width=8, height=6.5, units="in")
write_csv(mapdata, here::here("results", "iitax_change_hbars_numbers_data.csv"))

```


```{r include=TRUE}
# dotplot variant for the bar chart
df <- data3 %>%
  filter(!stabbr %in% c("US", "other"), stubnum==0, measure=="wtdsum", variable=="iitax", taxstatus=="allfilers") %>%
  select(stabbr, change) %>%
  mutate(change=-change / 1e9,
...
p <- df %>% 
  ...
  # geom_text_repel(aes(label=round(change, 1)), size=2) +
  # geom_text(aes(label=round(change, 1)), size=2) +
  geom_text(aes(x=ifelse(change > mdn, change + .4, change - .4), label=round(change, 1)), size=2, nudge_y = 0.1) +
  scale_y_discrete(name=NULL) +
  scale_x_continuous(name="Aggregate tax cut, $ billions", labels = scales::dollar_format()) +
  geom_vline(xintercept=mdn, linetype="solid", colour="darkgrey", size=1) +
  theme_minimal() +
  ...
  annotate(geom="text", x=mdn + .1, y=5, label="median", hjust=0) 
p
ggsave(plot=p, filename=here::here("results", "iitax_change_dotplot.png"), width=8, height=6.5, units="in")
write_csv(mapdata, here::here("results", "iitax_change_dotplot_data.csv"))
```
...
p <- df %>% 
  ...
  theme_minimal() +
  theme(axis.text.y = element_text(hjust = 0)) +
  ggtitle("Impact of repealing the SALT cap, average for taxable returns",
          subtitle="Compared to current 2021 law") +
...
write_csv(mapdata, here::here("results", "iitax_avgchange_hbars_data.csv"))
...
statetab1 <- function(stabbr){
  ...
  tabdata %>%
    gt()  %>%  
    tab_header(
      title = paste0("Total impact of SALT-cap repeal in $ billions: ", stname),
      subtitle = "Compared to current 2021 law"
    ) %>%
   cols_label(
      stub = "",
...
statetab2 <- function(stabbr){
  ...
  tabdata %>%
    gt()  %>%  
    tab_header(
      title = paste0("Average impact of SALT-cap repeal, per taxpayer, on ", stname),
      subtitle = "Compared to current 2021 law"
    ) %>%
   cols_label(
      stub = "",

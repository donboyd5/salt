...
*   [Stage_II_targets.csv]() has important values for 2011-2031. Selected 2021 values given below
    +   Wages and Salaries: $1 Million and Over: 425,117,251; with the $1m+ break is on agi I get:
          +   $588.9076 for puf
...
basedf <- basedf %>% mutate(agibase=c00100, type="base")
reformdf <- reformdf %>% left_join(basedf %>% select(RECID, agibase), by="RECID") %>% mutate(type="reform")

stack <- bind_rows(basedf, reformdf) %>%
  left_join(usweights %>% select(RECID, filer2017, REWT2021), by="RECID") %>%
...
Basic checks on the national stacked base-reform file before going further:
...
check <- stack %>% 
  group_by(type) %>%
  summarise(n=n(),
            filern=sum(filer2017),
            weight_puf=sum(s006),
            weight_rwt=sum(REWT2021),
            filer_puf=sum(filer2017 * s006),
            filer_rwt=sum(filer2017 * REWT2021),
            agibase_puf=sum(agibase * s006) / 1e9,
            agibase_rwt=sum(agibase * REWT2021) / 1e9,
            c00100_puf=sum(c00100 * s006) / 1e9,
            c00100_rwt=sum(c00100 * REWT2021) / 1e9,
            iitax_puf=sum(iitax * s006) / 1e9,
            iitax_rwt=sum(iitax * REWT2021) / 1e9)

check %>%
  kbl()

check <- stack %>% 
  select(type, stubnum, stub, iitax, s006, REWT2021) %>%
  pivot_longer(cols=c(s006, REWT2021), names_to = "wtype", values_to = "weight") %>%
  group_by(type, stubnum, stub, wtype) %>%
  summarise(iitax=sum(iitax * weight) / 1e9, .groups="drop")

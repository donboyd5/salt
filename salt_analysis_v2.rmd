...
usweights <- read_csv(paste0(SALTDIR, 'weights2021.csv'))
glimpse(usweights)
usweights %>%
  group_by(filer2017) %>%
...
basedf <- read_parquet(paste0(SALTDIR, "base_2021.parquet"),  # OLD: base2021.parquet
...
reformdf <- read_parquet(paste0(SALTDIR, reform_file),  # OLD: reform2021.parquet
                       col_select = all_of(vars)) %>%
...
tab <- tabdata %>%
  ...
  tab_header(
    title = paste0(reform_name, " compared to current 2021 law"),
    subtitle = html("Aggregate tax liabilities by income group ($ billions)")
  ) %>%
  cols_label(
      stub = "",
      wtdsum_base = "2021 current law",
...
tab <- tabdata %>%
  ...
  tab_header(
    title = paste0(reform_name, " compared to current 2021 law"),
    subtitle = html("Average tax change per taxpayer")
  ) %>%
  cols_label(
      stub = "",
      avgval_base = "2021 current law",
...
tab <- tabdata %>%
  ...
  tab_header(
    title = paste0(reform_name, " compared to current 2021 law"),
    subtitle = "States ranked by impact of SALT-cap reform, $ billions"
  ) %>%
  cols_label(
...
tab <- tabdata %>%
  ...
  tab_header(
    title = paste0(reform_name, " compared to current 2021 law"),
    subtitle = "Impact of SALT-cap reform for the United States and for individual states"
  ) %>%
  cols_label(
    stname="",
...
```{r map_avg, message=FALSE, warning=FALSE, include=TRUE}
mapbase <- data3 %>%
...
p <- ggplot(data = mdata, aes(x = x, y = y, group = group)) +
  ...
  theme(legend.position = "right") +
  theme(legend.title = element_blank()) + 
  ggtitle(paste0(reform_name, " compared to current 2021 law"),
          subtitle="Average tax reduction per taxpayer by state") +
...
p <- ggplot(data = mdata, aes(x = x, y = y, group = group)) +
  ...
  ggtitle(paste0(reform_name, " compared to current 2021 law"),
          subtitle="Aggregate tax reductions by state ($ millions)")
p
...
p <- df %>% 
  ...
  ggtitle(paste0(reform_name, " compared to current 2021 law"),
          subtitle="Aggregate tax reductions by state ($ billions)") +
